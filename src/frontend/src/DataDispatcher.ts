...
import { CanDBPartition, ItemInfo, Streams } from "../../declarations/CanDBPartition/CanDBPartition.did"
...
export class AppData {
    ...
    /// `"t" | "v"` - time, votes,.
    static async create(agent: Agent, itemId: string, kind: "t" | "v"): Promise<AppData> {
        const obj = new AppData(agent, itemId);
    ...
    async creator(): Promise<Principal> {
        return Principal.fromText("aaaaa-aa"); // FIXME
    }
    ...
    private async aList(outerCanister, outerKey, opts?: {lowerBound?: string, limit?: number})
        : Promise<{order: string, id: ItemRef, item: ItemInfo}[]>
    {
        ...
        return items4.map(({order, id, item}) => ({
            order,
            id,
            item: (item[0].item.ownership as any).owned,
        }));
    }
    async subCategories(opts?: {lowerBound?: string, limit?: number}): Promise<{order: string, id: ItemRef, item: ItemInfo}[]> {
        const {lowerBound, limit} = opts !== undefined ? opts : {lowerBound: "", limit: 5};
    ...
    async superCategories(opts?: {lowerBound?: string, limit?: number}): Promise<{order: string, id: ItemRef, item: ItemInfo}[]> {
        const {lowerBound, limit} = opts !== undefined ? opts : {lowerBound: "", limit: 5};
        if (this.agent === undefined) {
            return undefined;
        }
        ...
        const stream = this.item.ownedCategory !== undefined
    ...
    async items(opts?: {lowerBound?: string, limit?: number}): Promise<{order: string, id: ItemRef, item: ItemInfo}[]> {
        const {lowerBound, limit} = opts !== undefined ? opts : {lowerBound: "", limit: 5};
        if (this.agent === undefined) {
            return undefined;
        }
    ...
    async comments(opts?: {lowerBound?: string, limit?: number}): Promise<{order: string, id: ItemRef, item: ItemInfo}[]> {
        const {lowerBound, limit} = opts !== undefined ? opts : {lowerBound: "", limit: 5};
        if (this.agent === undefined) {
            return undefined;
        }
    ...
    async antiComments(opts?: {lowerBound?: string, limit?: number}): Promise<{order: string, id: ItemRef, item: ItemInfo}[]> {
        const {lowerBound, limit} = opts !== undefined ? opts : {lowerBound: "", limit: 5};
        if (this.agent === undefined) {
            return undefined;
        }
